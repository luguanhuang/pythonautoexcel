...
class sportMainWindow(Ui_MainWindow, QMainWindow):
    ...
    def DisMainPartBody(self, ws, onepagebegin, valueinfo):
        ...
        alignment = copy.copy(ws["D"+nextbegin].alignment)
        alignment.wrapText=True
        if len(valueinfo['mainspec'].encode("gbk")) >= 26:
            ws["D"+nextbegin].alignment = alignment
            font2 = Font(size = 8,name='宋体')
            ws["D"+nextbegin].font = font2
        else:
            ws["D"+nextbegin].font = font1

        # ws["D"+nextbegin].alignment.wrap_text=True #= ws["D"+nextbegin].alignment.copy(wrapText=True)
        tmpcell = 'D'+nextbegin+":E"+nextbegin
        ws.merge_cells(tmpcell)
        
        # 计算文本的像素宽度
        # text_width = ws["D"+nextbegin].column_width * ws["D"+nextbegin].font.size  # 假设单元格宽度和字体大小是按像素来的
        # print("mainspec realLength=", realLength(valueinfo['mainspec']), " lennn=", len(valueinfo['mainspec'].encode("gbk")))

        
        ws["F"+nextbegin] = valueinfo['mainsize']
        # print("mainsize=", valueinfo['mainsize'], " len=", len(valueinfo['mainsize']))
        # ws["F"+nextbegin].alignment = ws["F"+nextbegin].alignment.copy(wrapText=True)
        alignment = copy.copy(ws["F"+nextbegin].alignment)
        alignment.wrapText=True
        if len(valueinfo['mainsize'].encode("gbk")) >=18:
            ws["F"+nextbegin].alignment = alignment
            font2 = Font(size = 8,name='宋体')
            ws["F"+nextbegin].font = font2
        else:
            ws["F"+nextbegin].font = font1
        # ws["F"+nextbegin].alignment = alignment
        
        # print("mainsize realLength=", realLength(valueinfo['mainsize']), " lennn=", len(valueinfo['mainsize'].encode("gbk")))
        # text_width = ws["F"+nextbegin].column_width * ws["F"+nextbegin].font.size  # 假设单元格宽度和字体大小是按像素来的
        # print("text_width1=", text_width, " realLength=", )
       
        font2 = Font(bold=True, size = 9,name='宋体')
        ws["G"+nextbegin] = valueinfo['mainnotes']
        # print("mainnotes=", valueinfo['mainnotes'], " len=", len(valueinfo['mainnotes']))
        # ws["G"+nextbegin].alignment = ws["G"+nextbegin].alignment.copy(wrapText=True)
        alignment = copy.copy(ws["G"+nextbegin].alignment)
        alignment.wrapText=True
        if len(valueinfo['mainnotes'].encode("gbk")) >= 16:
            ws["G"+nextbegin].alignment = alignment
            font2 = Font(bold=True, size = 8,name='宋体')
            ws["G"+nextbegin].font = font2
        else:
            ws["G"+nextbegin].font = font2
        # ws["G"+nextbegin].alignment = alignment
        # ws["G"+nextbegin].font = font2
        # print("mainnotes realLength=", realLength(valueinfo['mainnotes']), " lennn=", len(valueinfo['mainnotes'].encode("gbk")))
        # text_width = ws["G"+nextbegin].column_width * ws["G"+nextbegin].font.size  # 假设单元格宽度和字体大小是按像素来的
        # print("text_width2=", text_width)
